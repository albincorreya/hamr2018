...
import madmom as mm
import cv2
from create_video import compute_activations
...
# parameters for adjusting the influence of the gravity and the electrostatic force
l_gravity = 1e-2
l_coulomb = 1e-6
...
def background(audio_file, fps=100):
...
def drum_background(audio_file):
    ...
    cluster_map = {i: j for i, j in zip(set(range(5)) - set([exclude_cluster]),
                                        range(4))}
...
def main(audio_file, write_frames=False, inverse=False):
    ...
    background_frames = background(audio_file)

    for i, (act, bg_img) in tqdm(enumerate(zip(activations, background_frames))):
        ...
        if i % 5 == 0:
            ...
            plt.scatter(particles[neg, 0], particles[neg, 1], c=neg_color, s=np.abs(act[neg]) * 1000 + 10)
            plt.scatter(particles[pos, 0], particles[pos, 1], c=pos_color, s=np.abs(act[pos]) * 1000 + 10)
            ...
            for index, hist in enumerate(particle_history):
                ...
                for h_idx, h in enumerate(hist):
                    ...
                    plt.scatter(h[0], h[1], c=color, s=size, alpha=1-(h_idx+1)*(1.0 / max_history))
